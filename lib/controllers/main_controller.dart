...
@override
  void onReady() {
    ...
    exploretrackyourprogress.bindStream(exploreStream("trackyourprogress"));
    exploremedical.bindStream(exploreStream("medical"));
    explorenutrition.bindStream(exploreStream("nutrition"));
    explorefertility.bindStream(exploreStream("fertility"));
    exploreexercises.bindStream(exploreStream("exercises"));
    explorecommunityforums.bindStream(exploreStream("communityforums"));
  ...
  createExplore(image,title,content)async{
    ...
    try{
      await firestore.collection("Explore").add({
        "userId":userId,"image":image,"title":title,"content":content,"created":DateTime.now().microsecondsSinceEpoch
      });
  ...
  updateExplore(image,title,content)async{
    ...
    try{
      await firestore.collection("Explore").doc(activitiesSelectId).update({
        "userId":userId,"image":image,"title":title,"content":content
      });
      Utils.showSuccess("Success!");
    }
    catch(e){
  ...
  Stream<Map<String,dynamic>>exploreStream(category){
    var ref =FirebaseFirestore.instance.collection("Explore").where('category', isEqualTo: category).orderBy("created").snapshots();
    return ref.map((list){
...
//creating data
  createActivities(time,date,title,contacts,description)async{
    ...
    try{
      ...
      await firestore.collection("Activities").add({
        "userId":userId,"date":date,"time":time,"title":title,"contacts":contacts,"decription":description,"created":DateTime.now().microsecondsSinceEpoch
      });
      Utils.showSuccess("Success!");
    }
    catch(e){
  ...
  updateActivities(title,date,time,description,content)async{
    ...
    try{
      ...
      await firestore.collection("Activities").doc(activitiesSelectId).update({
        "userId":userId,"date":date,"title":title,"content":content,"time":time,"description":description,
      });
      Utils.showSuccess("Success!");
    }
    catch(e){
  ...
  Stream<Map<String,dynamic>>activitiesStream(){
    var ref =FirebaseFirestore.instance.collection("Activities").where('userId', isEqualTo: AuthController.to.auth.currentUser.uid).orderBy("created").snapshots();
    return ref.map((list){
      return {for (var element in list.docs) element.id: element.data()};
